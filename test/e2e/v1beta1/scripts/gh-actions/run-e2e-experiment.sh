...
for exp_name in "${EXPERIMENT_FILE_ARRAY[@]}"; do
  ...
  ../../bin/run-e2e-experiment "$exp_path" || (kubectl get pods -n kubeflow && kubectl describe pods -n kubeflow && kubectl logs -n kubeflow -l katib.kubeflow.org/experiment=random && exit 1)
done
